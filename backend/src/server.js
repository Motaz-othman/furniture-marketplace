...
import { apiLimiter, readLimiter, createLimiter } from './shared/middleware/rateLimiter.js';
...
if (process.env.JWT_REFRESH_SECRET && process.env.JWT_REFRESH_SECRET.length < 32) {
  const msg = 'JWT_REFRESH_SECRET is too weak. Set a strong random string (min 32 chars).';
  if (isProduction) { console.error(`❌ ${msg}`); process.exit(1); }
  else console.warn(`⚠️  ${msg}`);
}
...
// Webhook must be registered here (not in payments.routes.js) because it needs
// express.raw() before express.json() is applied globally — Stripe signature
// verification requires the unmodified raw request body.
...
app.use('/api/products', readLimiter, productsRoutes);
app.use('/api/categories', readLimiter, categoriesRoutes);
app.use('/api/cart', createLimiter, cartRoutes);
app.use('/api/orders', createLimiter, ordersRoutes);
...
app.use('/api/checkout', createLimiter, checkoutRoutes);
app.use('/api', variantsRoutes);
app.use('/api/addresses', createLimiter, addressesRoutes);
