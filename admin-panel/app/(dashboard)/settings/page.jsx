...
export default function SettingsPage() {
  ...
  useEffect(() => {
    if (data) {
      setForm({
        ...
        deliveryPricing: {
          smallParcel: data.deliveryPricing?.smallParcel?.length > 0
            ? data.deliveryPricing.smallParcel
            : DEFAULT_DELIVERY_PRICING.smallParcel,
          ltl: data.deliveryPricing?.ltl?.length > 0
            ? data.deliveryPricing.ltl
  ...
  function updateDeliveryOption(tier, index, field, value) {
    setForm(f => {
      const options = [...(f.deliveryPricing[tier] || [])];
      options[index] = { ...options[index], [field]: value };
      return { ...f, deliveryPricing: { ...f.deliveryPricing, [tier]: options } };
    });
  }

  function addDeliveryOption(tier) {
    setForm(f => {
      const options = [
        ...(f.deliveryPricing[tier] || []),
  ...
  function removeDeliveryOption(tier, index) {
    setForm(f => {
      const options = f.deliveryPricing[tier].filter((_, i) => i !== index);
      return { ...f, deliveryPricing: { ...f.deliveryPricing, [tier]: options } };
    });
  }

  function resetDeliveryPricing() {
    setForm(f => ({ ...f, deliveryPricing: DEFAULT_DELIVERY_PRICING }));
  }
        ...
        <CardHeader>
          <div className="flex items-center justify-between">
            <div>
              ...
              <CardDescription>
                Options shown to customers at checkout. Small Parcel applies to rugs and small items; LTL applies to furniture and oversize pieces.
              </CardDescription>
            </div>
            <Button variant="outline" size="sm" onClick={resetDeliveryPricing}>
              Reset to Defaults
            </Button>
          </div>
        </CardHeader>
          ...
          {[
            {
              tier: 'smallParcel',
              label: 'Small Parcel',
              hint: 'Rugs and small accessories (shipType: Small Parcel / GROUND)',
            },
            {
              tier: 'ltl',
              label: 'LTL / Large Items',
              hint: 'Furniture and oversize items (shipType: LTL / GROUND - OVERSIZE)',
            },
          ].map(({ tier, label, hint }) => (
            <div key={tier}>
              <div className="flex items-center justify-between mb-3">
                ...
                <Button
                  ...
                  onClick={() => addDeliveryOption(tier)}
                >
                  <Plus className="h-3.5 w-3.5" /> Add Option
                </Button>
              </div>
              <div className="space-y-3">
                {(form.deliveryPricing?.[tier] || []).map((opt, i) => (
                  <div key={opt.key || i} className="grid grid-cols-[1fr_2fr_90px_36px] gap-2 items-end">
                    <div className="space-y-1">
                      {i === 0 && <Label className="text-xs">Label</Label>}
                      <Input
                        value={opt.label}
                        onChange={e => updateDeliveryOption(tier, i, 'label', e.target.value)}
                    ...
                    <div className="space-y-1">
                      ...
                      <Input
                        ...
                        onChange={e => updateDeliveryOption(tier, i, 'description', e.target.value)}
                    ...
                    <div className="space-y-1">
                      ...
                      <Input
                        ...
                        step="1"
                        value={opt.price}
                        onChange={e => updateDeliveryOption(tier, i, 'price', Number(e.target.value))}
                        placeholder="0"
                      />
                    </div>
                    <Button
                      ...
                      onClick={() => removeDeliveryOption(tier, i)}
                    >
                      <Trash2 className="h-4 w-4" />
                    </Button>
                  </div>
                ))}
                {(form.deliveryPricing?.[tier] || []).length === 0 && (
